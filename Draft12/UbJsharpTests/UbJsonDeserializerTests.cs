...
namespace UbJsharpTests
{
    class UbJsonDeserializerTests
    {
        ...
        {
            Assert.That(() => _deserializer.Deserialize(NoopBytes),
        ...
        {
            Assert.That(() => _deserializer.Deserialize(new byte[] { UbJsonCommon.StringMarker, UbJsonCommon.CharMarker, 0x01, 0x41 }),
        ...
        {
            Assert.That(() => _deserializer.Deserialize(new byte[] { UbJsonCommon.StringMarker, UbJsonCommon.Int64Marker, 0x7E, 0x9D, 0x07, 0x9C, 0x8F, 0x54, 0x5F, 0x15, 0x41 }),
        ...
        {
            Assert.That(() => _deserializer.Deserialize(new byte[] { 0x20 }),
                Throws.TypeOf<InvalidDataException>());
        }
        ...
        [Test]
        public void ShouldDeserializeStcArrayOfBytes()
        {
            var list = new List<byte> { 0x41, 0x42, 0x43 };

            var bytes = new List<byte>();
            bytes.AddRange(new byte[] { UbJsonCommon.ArrayStartMarker, UbJsonCommon.Int8Marker, 0x03, UbJsonCommon.UInt8Marker, 0x41, 0x42, 0x43 });

            Assert.That(_deserializer.Deserialize(bytes.ToArray()), Is.EqualTo(list));
        }

        [Test]
        public void ShouldDeserializeStcArrayOfBoolean()
        {
            var list = new List<bool> { true, false, true };

            var bytes = new List<byte>();
            bytes.AddRange(new byte[] { UbJsonCommon.ArrayStartMarker, UbJsonCommon.Int8Marker, 0x03, UbJsonCommon.FalseMarker, 0x54, 0x46, 0x54 });

            Assert.That(_deserializer.Deserialize(bytes.ToArray()), Is.EqualTo(list));
        }

        // FIXME -- test noop stc with mixed types (ints, lists, etc)
        // FIXME -- test DictionaryOfStcOfDictionary and many other combinations, just to make sure
